 ...
 * Cases when Decode/RF stage stalls are followings:
 *   - current instruction is NOT STORE (store_ins = 0), and both or one of its source registers depends on rs1 of LOAD
 *   - current instruction is STORE (store_ins - 0), and rs2 of STORE depends on rs1 of LOAD
 * If rs1 of STORE depends on rs1 of LOAD, them Decode/RF does NOT stalls, but LOAD forwards rs1 value to STORE in EX stage when LOAD is in MEM stage
 */
int
get_source_values(APEX_CPU* cpu, int rs2_exist, int store_ins)
{
...
decode(APEX_CPU* cpu)
{
  ...
  if (!stage->busy && !stage->stalled) {
    ...
    if (strcmp(stage->opcode, "STORE") == 0) {
      cpu->store_rs1_valid = 1;
      get_source_values(cpu, 1, 1);
    }

    /* Read data from register file for LOAD */
    if (strcmp(stage->opcode, "LOAD") == 0) {
      get_source_values(cpu, 0, 0);
    }
...
execute(APEX_CPU* cpu)
{
  ...
  if (!stage->busy && !stage->stalled) {
    ...
    if (strcmp(stage->opcode, "STORE") == 0) {
      ...
      /* store_rs1_valid = 0 means that rs1 of STORE is not valid, because
       * previous instruction is LOAD, and it uses rs1 as rd (destination).
       * In this case, STORE gets value of rs1 from MEM stage when LOAD loads
       * data for rs1 from memory.
       */
      if (!cpu->store_rs1_valid) {
        printf("got_from_mem,");
